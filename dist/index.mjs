...
const point = {
  ...
  total(msg) {
    console.log(chalk.white("\u{1F6A5}", msg));
  },
  message(msg) {
    console.log(chalk.green("\u{1F508}", msg));
  },
  warn(msg) {
    console.log(chalk.yellow("\u{1F6A7}", msg));
  },
  error(msg) {
    console.log(chalk.red("\u{1F47B}", msg));
  },
  success(msg) {
    console.log(chalk.white("\u{1F389}", msg));
  },
  query(msg) {
    console.log(chalk.yellow("\u{1F50E}", msg));
  }
};
    ...
    input: async () => {
      const { text } = await enquirer.prompt({
        ...
        validate(value) {
          ...
          if (failed.length) {
            return [`\u{1F6A7} \u6821\u9A8C\u5931\u8D25`, chalk.yellow(failed.join("\n"))].join("\n");
          }
          return true;
        }
      });
...
const parset = async (config) => {
  ...
  if (usage.length === 0 || changed) {
    usage = await new TreeSelectPrompt({
      ...
      header: changed ? chalk.yellow("\u{1F6A7} ", chalk.white("apifox \u6587\u6863\u53D1\u751F\u4E86\u53D8\u5316, \u8BF7\u91CD\u65B0\u786E\u8BA4\u751F\u6210\u5185\u5BB9")) : void 0,
...
const findSchemaFilePath = (schema2) => {
  ...
  if (schema2 && fs.existsSync(schema2))
    return schema2;
  const dir = dirs.find((dir2) => fs.existsSync(dir2));
  return dir || url;
};
const inputConfigAndLoadApis = async (config) => {
  ...
  if (isUndefined(output.applyImportStatements)) {
    ...
    let n = 0;
    while (searchPath !== np$1.dirname(searchPath) && n < 10) {
      res = glob.sync(query, { cwd: searchPath });
      if (res.length > 0) {
        break;
      }
      searchPath = np$1.dirname(searchPath);
      n++;
    }
    const projectPkg = fs.readFileSync(np$1.join(rootPath, "package.json"), { encoding: "utf-8" });
    let statement = "import axios from 'axios'";
    if (res.length > 0) {
      const requestPath = res[0];
      const requestFile = fs.readFileSync(np$1.join(searchPath, requestPath), { encoding: "utf-8" });
      let importPath = np$1.relative(output.dir, np$1.join(searchPath, requestPath)).replace(/\\/g, "/");
      importPath = importPath.replace(np$1.extname(importPath), "");
...
`;
  ...
  return compile(jsonSchema, typeName, JSONSchemaGenerateOptions);
};
...
const createFunctionResponseInterface = async (api, functionName, config) => {
  ...
  fs.writeFileSync("./ro.json", JSON.stringify(ro, null, 4), { encoding: "utf-8" });
...
const transformDts = (code) => {
  const transformers = typescript.getTransformers({
    target: typescript.ScriptTarget.ESNext,
...
const writeFile = (path, code, config) => {
  ...
  if (config.output.clear && fse.existsSync(dir))
    fse.removeSync(dir);
  ...
  const basename = np.basename(path, np.extname(path));
  code = lint(path, code);
...
const createIndexFile = (groupApi, config) => {
  const indexFilePath = np.normalize(np.join(config.output.dir, "index.ts"));
...
process.addListener("exit", (code) => {
  if (code === 200 && config && configFilePath) {
    const output = prettier.format(JSON.stringify(config), {
      parser: "json",
      ...prettierConfig
    });
...
program.name(pkg.name).version(pkg.version).description(pkg.description).helpOption(void 0, "\u67E5\u770B\u5E2E\u52A9").option("-r, --reset <reset project>", "\u662F\u5426\u91CD\u7F6E\u914D\u7F6E\u6587\u4EF6").option("-c, --config <config file path>", "\u6307\u5B9A\u4F7F\u7528\u7684\u914D\u7F6E\u6587\u4EF6").action(async ({ reset, config: customConfigPath }) => {
