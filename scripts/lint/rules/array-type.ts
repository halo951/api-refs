 ...
 */
function isSimpleType(node: TSESTree.Node & { typeArguments?: any; typeName?: any }): boolean {
    switch (node.type) {
        ...
        case AST_NODE_TYPES.TSTypeReference:
            if (node.typeName && node.typeName.type === AST_NODE_TYPES.Identifier && node.typeName.name === 'Array') {
...
type MessageIds = 'errorStringArray' | 'errorStringArraySimple' | 'errorStringGeneric' | 'errorStringGenericSimple'
    ...
    create(context, [options]) {
        ...
        return {
            TSArrayType(node): void {
                const isReadonly =
                    node.parent.type === AST_NODE_TYPES.TSTypeOperator && node.parent.operator === 'readonly'
            ...
            TSTypeReference(node): void {
                ...
                const parentParens =
                    ...
                    !eslintUtils.isParenthesized(node.parent.elementType, sourceCode)
